...
procedure Evaluer_Alea_LCA is
	...
	begin
		...
		-- Effectue Taille tirages aléatoires d'entiers (compris entre 1 et Borne) et stocke les résultats dans liste
		for i in 1..Taille loop

			Get_Random_Number(tirage);
			

			if Cle_Presente(liste,tirage) then
				val := La_Valeur(liste,tirage) + 1;
				Enregistrer(liste,tirage,val);
			else	
				Enregistrer(liste,tirage,1);
			end if;
		...
		for j in 1..Borne loop 
			...
			if Cle_Presente(liste,j) then

				if La_Valeur(liste,j) > Max then 
					Max := La_Valeur(liste,j);
				end if;

				if La_Valeur(liste,j) < Min then 
					Min := La_Valeur(liste,j);
				end if;
			else 
				Min := 0;
			end if;
